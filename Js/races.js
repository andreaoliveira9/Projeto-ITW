var vm = function() {
    ...
    search = function() {
        ...
        var changeuri = 'http://192.168.160.58/Formula1/api/Search/Races?q=' + self.search;
        self.raceslist = [];
        ajaxHelper(changeuri, 'GET').done(function(data) {
            ...
            if (self.filter != 0) {
                ...
                for (var info in auto) {
                    self.raceslist.push(auto[info]);
                }
            } else {
                self.records(data);
                self.totalRecords(data.length);
                for (var info in data) {
                    self.raceslist.push(data[info]);
                }
            }
    ...
    $(".countryFilter").change(function() {
        ...
        p = parseInt($(this).children("option:selected").val());
        self.filter = p;
        if (p != 0) {
            showLoading();
            var url = '';
            if (self.search != '') {
                url = 'http://192.168.160.58/Formula1/api/Search/Races?q=' + self.search;
            } else {
                url = self.baseUri();
            }
    ...
    self.favorites = ko.observableArray(JSON.parse(localStorage.getItem("raceFavorites")));

    self.favButton = (id, event) => {
        if (!event.target.classList.contains('active')) {
            if (self.favorites.indexOf(id) === -1)
                self.favorites.push(id)
            self.updateLocalStorage("raceFavorites", self.favorites())
